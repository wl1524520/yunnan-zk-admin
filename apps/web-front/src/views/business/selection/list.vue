...
async function loadOptions() {
  ...
  loading.value = true;
  try {
    options.value = await getItemOptions(academicTermId.value, studentId.value);
    existing.value = await getConfirmedSelections(
    ...
    mode.value = existing.value.length > 0 ? 'correct' : 'confirm';
  } finally {
    loading.value = false;
  }
}
...
watch([academicTermId, studentId], () => {
...
<template>
  <Page
    title="选测确认"
    description="按学生绑定的规则版本展示该学期、年级和性别适用的候选项目。"
  >
    <Card>
      <div class="mb-5 flex flex-wrap items-end gap-3">
        <FilterForm />
        <Button :loading="loading" type="primary" @click="loadOptions">
          读取可选项目
        </Button>
      </div>
      ...
      <template v-if="options">
        ...
        <Alert
          ...
          type="info"
          show-icon
          message="该学生本学期没有需要确认的选测组。"
        />
        <div v-else class="mt-6 border-t pt-4">
          <Space class="mb-3">
            <Button
              :type="mode === 'confirm' ? 'primary' : 'default'"
              @click="mode = 'confirm'"
            >
              首次确认
            </Button>
            <Button
              :type="mode === 'correct' ? 'primary' : 'default'"
              @click="mode = 'correct'"
            >
              选项纠错
            </Button>
